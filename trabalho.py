...
# Lê CSV ou cria DataFrame vazio
if os.path.exists(vazio):
    DF = pd.read_csv(vazio)
else:
...
if col3.button('Funções Avançadas'):
    ...
    ler_g = pd.read_csv(vazio)
    ler_g['Peças Produzidas'] = ler_g['Peças Produzidas'].astype(int) # converter em int
